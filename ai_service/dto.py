...
class DatasetConfig(BaseModel):
    source_uri: str = Field(
        ..., description="Path or URI to the dataset (e.g., local folder or S3)"
    )
    num_classes: PositiveInt = Field(
        ..., ge=2, description="Number of classes in the dataset"
    )
    batch_size: PositiveInt = Field(
        default=64, ge=1, description="Number of images per batch"
    )
...
class ModelConfig(BaseModel):
    architecture: str = Field(
        ..., description="Model variant ID from the registry (e.g., resnet50)"
    )
    pretrained: bool = Field(
        default=True, description="Whether to start with pre-trained weights"
    )
    drop_rate: NonNegativeFloat = Field(
...
class OptimizationConfig(BaseModel):
    ...
    weight_decay: NonNegativeFloat = Field(
        default=0.05, ge=0.0, description="Weight decay for regularization"
    )


class ScheduleConfig(BaseModel):
    epochs: PositiveInt = Field(
        default=50, ge=1, description="Total number of training epochs"
    )
...
class TrainRequest(BaseModel):
    ...
    webhook_url: str | None = Field(
        None,
        description="Optional webhook URL to receive progress and completion events",
    )
...
class ExportRequest(BaseModel):
    ...
    num_classes: int = Field(
        ..., description="Number of output classes the model was trained on"
    )
    export_format: str = Field(
        default="onnx",
        pattern="^(onnx|torchscript)$",
        description="Target format for export",
    )
    webhook_url: str | None = Field(
        default=None, description="Optional webhook URL to receive completion events"
    )
