...
from theseus.backends.ludwig.evaluate import evaluate as build_evaluation_report
from theseus.services.evaluate import MAX_TOP_ERRORS
...
def test_category_report_top_errors_capped_at_max():
    ...
    result = build_evaluation_report(model, df, SPLIT_COL, ITEM_ID_COL)
    assert result is not None
    assert len(result.report["topErrors"]) == MAX_TOP_ERRORS
