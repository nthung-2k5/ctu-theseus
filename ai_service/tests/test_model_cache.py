...
def cache(monkeypatch):
    ...
    of a real S3 download + `LudwigModel.load()` — these tests are about
    the cache's own concurrency/eviction behavior, not Ludwig."""
    load_calls: list[str] = []

    def fake_load_sync(run_id: str):
...
async def test_concurrent_requests_for_same_run_load_once(cache):
    ...
    `LudwigModel.load()` — the per-run lock, not N duplicate loads."""
